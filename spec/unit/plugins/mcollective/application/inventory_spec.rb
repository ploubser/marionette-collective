...
module MCollecitve
    describe "inventory application" do
        before do
            application_file =  File.join([File.dirname(__FILE__), "../../../../../plugins/mcollective/application/inventory.rb"])
            @util = MCollective::Test::ApplicationTest.new("inventory", :application_file => application_file)
            @app = @util.plugin
        end
        ...
        describe "#node_inventory" do
           it "should raise an exception if it cannnot retrieve stats from the node" do
                ...
                IO.any_instance.expects(:puts).with("Failed to retrieve daemon_stats from test: RPC ABORTED")
                @app.main
           end
           ...
           it "should display and error message if it cannot retrieve the inventory for a node" do
                ...
                IO.any_instance.expects(:puts).with("Failed to retrieve inventory for test: RPC ABORTED")
                @app.main
           end
           ...
           it "should print an error message if an exception is raised" do
               rpcclient_mock = mock
                ...
                IO.any_instance.expects(:puts).with("Failed to display node inventory: Exception: ERROR")
                @app.main
           end
        ...
        describe "#formatted_inventory" do
            ...
            it "should raise an exception if no block is given" do
                IO.any_instance.expects(:puts).with("Could not create report: RuntimeError: Need to give a block to formatted_inventory")
                expect{
            ...
            it "should raise exception if page body format is not defined" do
                IO.any_instance.expects(:puts).with("Could not create report: RuntimeError: Need to define page body format")
                expect{
