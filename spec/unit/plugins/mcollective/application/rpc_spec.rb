...
module MCollective
    describe "rpc application" do
        before do
            application_file = File.join([File.dirname(__FILE__), "../../../../../plugins/mcollective/application/rpc.rb"])
            @util = MCollective::Test::ApplicationTest.new("rpc", :application_file => application_file)
            @app = @util.plugin
        end
        ...
        describe "#post_option_parser" do
            it "should termiate with error if agent and action are not both specified and commandline arguments are less than two" do
                IO.any_instance.expects(:puts).with("No agent, action and arguments specified")
            ...
            it "should display an error message if agent and action are not both defined and commandline arguments do not match the given regex" do
                ...
                ARGV << "action"
                ARGV << "args"
                IO.any_instance.expects(:puts).with("Could not parse --arg args")
