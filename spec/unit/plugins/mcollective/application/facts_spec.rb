...
module MCollective
    describe "facts aplication" do
        before do
            application_file = File.join([File.dirname(__FILE__), "../../../../../plugins/mcollective/application/facts.rb"])
            @util = MCollective::Test::ApplicationTest.new("facts", :application_file => application_file)
            @app = @util.plugin
        end
        ...
        describe "#main" do
            it "should handle failure responses correctly" do
                IO.any_instance.expects(:puts).with("Could not parse facts for node2: NoMethodError: undefined method `[]' for nil:NilClass")
