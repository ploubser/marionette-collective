...
module MCollective
    ...
    describe "find application" do
        before do
            application_file = File.join([File.dirname(__FILE__), "../../../../../plugins/mcollective/application/find.rb"])
            @util = MCollective::Test::ApplicationTest.new("find", :application_file => application_file)
            @app = @util.plugin
        end
