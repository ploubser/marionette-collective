...
module MCollective
    ...
    describe "controller application" do
        ...
        before do
            application_file = File.join([File.dirname(__FILE__), "../../../../../plugins/mcollective/application/controller.rb"])
            @util = MCollective::Test::ApplicationTest.new("controller", :application_file => application_file)
            @app = @util.plugin
        end
        ...
        describe "#print_statistics" do
            ...
            it "should print statistics" do
                ...
                @app.stubs(:configuration).returns(:command => "stats")
                @mock_client.expects(:discovered_req).with("stats", "mcollective").yields(:senderid => "node1", :value => "1", :body => {:stats => {:total => 0, :replies => 0, :validated => 0, :unvalidated => 0, :filtered => 0, :passed => 0}})
